...
static void print_reg_or_data_2(command_t* command, int immediate, int start) {
  if(immediate)
    ...
    fprintf(MSG_OUT, "g[%" PRIu8 "]", (unsigned char)vm_getbits(command, start - 4, 4));
}
...
static void print_special_instruction(command_t* command) {
  ...
  switch(op) {
    case 0: /*  NOP */
      ...
      break;
    case 1: /*  Goto line */
      fprintf(MSG_OUT, "Goto %" PRIu8, (unsigned char)vm_getbits(command, 7, 8));
      break;
    case 2: /*  Break */
    ...
    case 3: /*  Parental level */
      fprintf(MSG_OUT, "SetTmpPML %" PRIu8 ", Goto %" PRIu8,
		(unsigned char)vm_getbits(command, 11, 4), (unsigned char)vm_getbits(command, 7, 8));
      break;
    default:
...
static void print_linksub_instruction(command_t* command) {
  ...
  if(linkop < sizeof(link_table)/sizeof(link_table[0]))
    fprintf(MSG_OUT, "%s (button %" PRIu8 ")", link_table[linkop], (unsigned char)button);
  else
...
static void print_link_instruction(command_t* command, int optional) {
  ...
  switch(op) {
    ...
    case 1:
      ...
      break;
    case 4:
      fprintf(MSG_OUT, "LinkPGCN %" PRIu16, (unsigned short)vm_getbits(command, 14, 15));
      break;
    case 5:
      fprintf(MSG_OUT, "LinkPTT %" PRIu16 " (button %" PRIu8 ")",
	      (unsigned short)vm_getbits(command, 9, 10), (unsigned char)vm_getbits(command, 15, 6));
      break;
    case 6:
      fprintf(MSG_OUT, "LinkPGN %" PRIu8 " (button %" PRIu8 ")",
	      (unsigned char)vm_getbits(command, 6, 7), (unsigned char)vm_getbits(command, 15, 6));
      break;
    case 7:
      fprintf(MSG_OUT, "LinkCN %" PRIu8 " (button %" PRIu8 ")",
	      (unsigned char)vm_getbits(command, 7, 8), (unsigned char)vm_getbits(command, 15, 6));
      break;
    default:
...
static void print_jump_instruction(command_t* command) {
  switch(vm_getbits(command, 51, 4)) {
    case 1:
      ...
      break;
    case 2:
      fprintf(MSG_OUT, "JumpTT %" PRIu8, (unsigned char)vm_getbits(command, 22, 7));
      break;
    case 3:
      fprintf(MSG_OUT, "JumpVTS_TT %" PRIu8, (unsigned char)vm_getbits(command, 22, 7));
      break;
    case 5:
      fprintf(MSG_OUT, "JumpVTS_PTT %" PRIu8 ":%" PRIu16,
	      (unsigned char)vm_getbits(command, 22, 7), (unsigned short)vm_getbits(command, 41, 10));
      break;
    case 6:
      switch(vm_getbits(command, 23, 2)) {
        case 0:
          ...
          break;
        case 1:
          fprintf(MSG_OUT, "JumpSS VMGM (menu %" PRIu8 ")", (unsigned char)vm_getbits(command, 19, 4));
          break;
        case 2:
          fprintf(MSG_OUT, "JumpSS VTSM (vts %" PRIu8 ", title %" PRIu8
		  ", menu %" PRIu8 ")", (unsigned char)vm_getbits(command, 30, 7), (unsigned char)vm_getbits(command, 38, 7), (unsigned char)vm_getbits(command, 19, 4));
          break;
        case 3:
          fprintf(MSG_OUT, "JumpSS VMGM (pgc %" PRIu8 ")", (unsigned char)vm_getbits(command, 46, 15));
          break;
        }
      break;
    case 8:
      switch(vm_getbits(command, 23, 2)) {
        case 0:
          fprintf(MSG_OUT, "CallSS FP (rsm_cell %" PRIu8 ")",
              (unsigned char)vm_getbits(command, 31, 8));
          break;
        case 1:
          fprintf(MSG_OUT, "CallSS VMGM (menu %" PRIu8
		  ", rsm_cell %" PRIu8 ")", (unsigned char)vm_getbits(command, 19, 4), (unsigned char)vm_getbits(command, 31, 8));
          break;
        case 2:
        ...
        case 3:
		  ...
		  (unsigned char)vm_getbits(command, 46, 15), (unsigned char)vm_getbits(command, 31, 8));
          break;
      }
      break;
    default:
...
/*        Surely there must be some system set command for that ? */
  ...
  switch(vm_getbits(command, 59, 4)) {
    ...
    case 2: /*  Set system reg 9 & 10 (Navigation timer, Title PGC number) */
      ...
      fprintf(MSG_OUT, " ");
      print_system_reg(10);
      fprintf(MSG_OUT, " = %" PRIu16, (unsigned short)vm_getbits(command, 30, 15)); /*  ?? */
      break;
    ...
    case 6: /*  Set system reg 8 (Highlighted button) */
      ...
      if(vm_getbits(command, 60, 1)) /*  immediate */
        ...
        fprintf(MSG_OUT, " = g[%" PRIu8 "]", (unsigned char)vm_getbits(command, 19, 4));
      break;
    default:
